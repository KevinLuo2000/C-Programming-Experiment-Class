...
void Sort_pointer(transcript *head_,int N)//交换节点指针域
{
    ...
    pre_head->Next=node_;
    //node_->Prior=pre_head;
    for(i=0;i<1;i++)//遍历n次链表
    {
        node_=pre_head;
        for(j=0;j<N-i-1;j++)//链未结束时
        {
            printf("%d",node_->Next->AVG>node_->Next->Next->AVG);
            if(node_->Next->AVG>node_->Next->Next->AVG)//交换指针域
            {
                transcript *node1=node_->Next;
                transcript *node2=node1->Next;
                node1->Prior->Next=node2;
                node2->Prior=node1->Prior;
