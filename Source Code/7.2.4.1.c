...
void Sort_data(transcript *head_,int N)//交换节点数据域
{
    // transcript temp;
    // Bubble Sort
    int i,j;
    for(i=0;i<N;i++)//遍历n次链表
    {
        ...
        {
            if(node_->AVG>node_->Next->AVG)//交换数据域，挨个成员交换
            {
                Swap_float(&node_->AVG,&node_->Next->AVG);
                Swap_float(&node_->SUM,&node_->Next->SUM);
                Swap_float(&node_->GeneralPhysics,&node_->Next->GeneralPhysics);
                Swap_float(&node_->CProgramming,&node_->Next->CProgramming);
                Swap_float(&node_->HigherMaths,&node_->Next->HigherMaths);
                Swap_float(&node_->English,&node_->Next->English);
                Swap_string(node_->number,node_->Next->number);
                Swap_string(node_->name,node_->Next->name);
            }
            node_=node_->Next;//遍历指针node右移
        }
    }
